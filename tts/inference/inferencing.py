...
def _synthesize_audio(
    ...
    if use_vllm:
        # Note: In _generate_speech_tokens, we made vLLM return ONLY the newly generated token ids
        # wrapped in a torch.tensor. So generated_ids here does NOT contain the prompt.
        logging.info("DEBUG: vLLM generated %d new tokens", generated_ids.shape[0])
        
        # Remove the <|speech_end|> token from the end if it exists
        if len(generated_ids) > 0 and generated_ids[-1] == speech_end_id:
